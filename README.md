...
The widget checks the two accounts every 5 seconds by default. Click the interval button to cycle through `5s`, `1m`, and `P` (paused), then back to `5s`. The choice is remembered. Paused mode stops automatic updates, including updates when the window gains focus; click ↻ to refresh manually. The status stays still between readings. Super small shows the two percentages; hover over a limit to see its reset time. If a cookie file changes, it is reloaded on the next update. Cookie files must be JSON arrays in the browser export format (`name`, `value`, `domain`, and optional cookie fields). If a session expires, export fresh cookies to the same file.
